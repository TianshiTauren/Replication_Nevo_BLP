...
load mvalold  %mean utility values calculated from Pure logit
...
% If the change in the value of parameters (theta2) was low (<0.01), the
% tolerance level was set to 10E-9, while after that every additional 50
% iterations the tolerance level increases by an order of ten
...
if flag == 1 & max(isnan(mval)) < 1; %large change in parameters and no NaN in mval
   mvalold = mval;
   oldt2 = theta2;
